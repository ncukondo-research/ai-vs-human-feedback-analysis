...
| フェーズ | 完了 | 合計 | 進捗率 |
|---------|------|------|--------|
| Phase 1: 別リポジトリでの分析 | 0 | 12 | 0% |
| Phase 2: 原稿修正（高優先度） | 0 | 15 | 0% |
| Phase 3: 原稿修正（中優先度） | 0 | 14 | 0% |
...
- [ ] **評価者間信頼性の計算** (F8, N5)
  ...
  - Cohen's kappa または ICC を計算
  - 結果: _______________

- [ ] **効果量の計算** (G5)
  - フィードバック長さの差に対する Cohen's d
  - 結果: d = _______________
...
- [ ] **学生ログの特性** (F5, G1, G2)
  - 文字数: 範囲 ___ - ___, 平均 ___, SD ___
  - 形式: 自由記述のみ / 質問あり / リッカート尺度あり
  - 文字数制限: 上限 ___ / 下限 ___ / なし

- [ ] **欠測データの確認** (Editor 9)
  - 欠測の有無: あり / なし
  - 欠測割合: ___%
  - MCAR検定結果（必要な場合）: _______________
...
- [ ] **Methods にログ特性を追加**
  - 文字数範囲、平均、SD（Phase 1.2 の結果）
  - 形式（自由記述/質問/リッカート尺度）
  - 文字数制限の有無
  - スマホ/PC使用率（不明な場合はその旨記載）
...
- [ ] **信頼性指標の報告**（計算可能な場合）
  - Cohen's kappa または ICC を Results に追加

- [ ] **Limitations への記載**（計算不可能または低い場合）
  - 各データセットを1名ずつが評価した限界を記載
