...
![The process from preparing AI feedback and supervisor feedback to evaluating them](./assets/Fig_flow_of_feedback_and_evaluation.svg){#fig-flow_to_evaluation}
...
df <- read.csv("assets/dataset_summary.csv")
...
![Distribution of Scores for Each Category ](assets/fig_diff_ai_vs_supervisor.svg){#fig-diff-ai-vs-supervisor}
...
df <- read.csv("assets/paired_ttest_results_formatted.csv")
...
df <- read.csv("assets/clmm_summary_formatted_bonferroni.csv")
avail <- 6
w <- rep(avail /ncol(df), ncol(df))
df |> 
  flextable() |>
...
![Distribution of Mean Scores per Record](assets/fig_mean_score_per_record.svg){#fig-mean-score-per-record}
...
df <- read.csv("assets/correlation_letter_count_score_by_person_item_formatted.csv")
...
df <- read.csv("assets/theme_table_en_simple.csv") |>
  pivot_wider(
    names_from = provider,
...
df <- read.csv("assets/joint_display_analysis.csv") |>
...
GitHub Copilot (Ver. 0.35.2, AI models used: Claude Sonnet 4.5/Claude Opus4.5/GPT-4.1/GPT-5) was used to assist in writing R code for quantitative analysis. The editor's predictive code‑generation feature was used for in-editor assistance. All generated code was reviewed, tested, and modified by TK, and reviewed by JD. For manuscript writing, the same AI agent was used to assist with translation from Japanese into English. After entering content in Japanese in the editor, we prompted the in-editor chat with "translate to english" and used the generated English text as the basis for additions and revisions to the manuscript. All generated text was checked and edited by TK, and reviewed and revised by all authors. For assistance with literature searches, Deep Research by ChatGPT (GPT-4o, GPT-5, GPT-5.1) was used. All selected articles were reviewed by TK, and results from hand searches were also compared and incorporated. 
